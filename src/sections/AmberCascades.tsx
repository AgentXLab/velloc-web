...
function initAmberCascades(canvas: HTMLCanvasElement) {
  ...
  // ---- Official AI model logos (original-color SVG renders)
  const LOGO_FILES = [
    'openai', 'anthropic', 'kimi', 'qwen',
    'claude', 'gemini', 'deepseek', 'zai',
  ];
  ...
  const randomLogo = () => Math.floor(Math.random() * logoImages.length);
  ...
  function render(timestamp: number) {
    ...
    for (const col of columns) {
      ...
      const LOGO_GAP = FONT_SIZE * 0.4; // extra gap between the logo head and the glyphs below
      for (let j = 0; j < col.length; j++) {
        const charY = col.y - j * FONT_SIZE - (j > 0 ? LOGO_GAP : 0);
        ...
        const tint = `rgba(${r},${g},${b},${brightness})`;
        ctx.fillStyle = tint;
        if (j === 0) {
